...
//@return           desc                返回摘要内容
func (model *DocText) GetDescByMd5(md5str interface{}, length ...int) (desc string) {
	orm.NewOrm().QueryTable(GetTableDocText()).Filter("Md5", md5str).One(model)
	l := 255
	if len(length) > 0 {
		l = length[0]
	}
	return beego.Substr(model.Content, 0, l)
}
...
//@param            length              需要获取的长度
//@return           desc                返回摘要内容
func (model *DocText) GetDescByDsId(dsid interface{}, length ...int) (desc string) {
	if dsinfo, rows, _ := NewDocument().GetDocStoreByDsId(dsid); rows > 0 {
		return model.GetDescByMd5(dsinfo[0].Md5, length...)
	}
	return
}
...
//@param            length              需要获取的长度
//@return           desc                返回摘要内容
func (model *DocText) GetDescByDid(did interface{}, length ...int) (desc string) {
	var dsid = 0
