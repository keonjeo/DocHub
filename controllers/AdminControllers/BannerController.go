...
//横幅列表
func (controller *BannerController) Get() {
	var err error
	if controller.Data["Banners"], _, err = models.NewBanner().List(1, 100); err != nil && err != orm.ErrNoRows {
		helper.Logger.Error(err.Error())
	}
	controller.Data["IsBanner"] = true
	controller.TplName = "index.html"
}

//新增横幅
func (controller *BannerController) Add() {
	f, h, err := controller.GetFile("Picture")
	if err != nil {
		helper.Logger.Error(err.Error())
	...
	err = controller.SaveToFile("Picture", filePath) // 保存位置
	if err != nil {
		helper.Logger.Error(err.Error())
		controller.ResponseJson(false, err.Error())
	}
	...
	controller.ResponseJson(true, "横幅添加成功")
}

//删除横幅
func (controller *BannerController) Del() {
	var err error
